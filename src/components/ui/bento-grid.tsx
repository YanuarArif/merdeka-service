...
const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "grid w-full auto-rows-[10rem] lg:h-[40rem] grid-cols-4 gap-4",
        className
      )}
    >
      {children}
...
const BentoCard = ({
  ...
  >
    ...
    <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 p-4 transition-all duration-300 group-hover:-translate-y-10">
      {Icon && ( // Render Icon only if it's provided
        <Icon className="h-12 w-12 origin-left transform-gpu text-neutral-700 transition-all duration-300 ease-in-out group-hover:scale-75" />
      )}

      {/* <div className="bg-white dark:bg-black opacity-90 rounded-lg p-2">
